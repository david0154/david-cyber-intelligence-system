"""
Task Router v3 - All modules including new engines
...
class TaskRouter:
    ...
    def _safe_load(self, key, loader_fn):
        try:
            ...
            logger.warning(f"[Router] ⚠️ {key} unavailable: {e}")
    ...
    def _load_engines(self):
        ...
        # NEW v3 engines
        self._safe_load("vuln_scan", lambda: __import__('engines.vulnerability_scanner', fromlist=['VulnerabilityScanner']).VulnerabilityScanner())
        self._safe_load("soc", lambda: __import__('engines.soc_engine', fromlist=['SOCEngine']).SOCEngine())
        self._safe_load("attack_sim", lambda: __import__('engines.attack_sim_engine', fromlist=['AttackSimEngine']).AttackSimEngine())
        # Security tools
    ...
    def route(self, module: str, params: dict) -> dict:
        ...
        if engine is None:
            return {"status": "error",
                    "message": f"Module '{module}' not loaded. Run: pip install -r requirements.txt"}
        logger.info(f"[Router] → {module} | params={list(params.keys())}")
        try:
            dispatch = {
                ...
                "vuln_scan":   lambda: engine.full_scan(params.get("target", "")),
                "soc":         lambda: engine.analyze_log(
                                    params.get("log_text", ""),
                                    params.get("source", "manual")),
                "attack_sim":  lambda: engine.run_full_pentest(
                                    params.get("target", ""),
                                    params.get("scope", "basic")),
                ...
                "hydra":       lambda: engine.test(params.get("target", ""),
            ...
            logger.error(f"[Router] Crash in {module}: {e}")
